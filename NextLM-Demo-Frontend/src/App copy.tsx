...
import { useSearchAnalytics } from './hooks/useSearchAnalytics'; // New analytics hook
import { 
  loadSearchHistory, 
  saveSearchHistory, 
  clearSearchHistory,
...
function App() {
  ...
  const { logSearchActivity, logUserActivity } = useSearchAnalytics(); // New analytics hook
        ...
        if (user) {
          try {
            await logSearchActivity({
              search_query: query,
              search_type: 'knowledge_gpt',
              filters: { max_candidates: 2 }
            });
          } catch (analyticsError) {
            console.warn('Failed to log search activity:', analyticsError);
          }
        }
    ...
    if (user) {
      try {
        await logUserActivity({
          activity_type: 'toggle_tracking',
          description: `Toggled tracking for ${candidate.name} at ${candidate.company}`
        });
      } catch (analyticsError) {
        console.warn('Failed to log user activity:', analyticsError);
      }
    }
          ...
          <div className="fixed bottom-12 left-6 z-30 flex items-center space-x-3">
            ...
            {searchHistory.length > 0 && !showHistory && (
              <button
                ...
                className="flex items-center space-x-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-4 py-3 text-white hover:bg-white/20 transition-all duration-200 shadow-lg"
              >
                <History size={16} />
